...
var chart = {
  ...
  perspectiveOffsetX: 8,
  perspectiveOffsetY: 5,
  ...
  init: function() {
      ...
      var yearAve = _.reduce(data, function(m, v) {return m + v;}, 0) / 7;
      var yearMax = _.max(data);
      var yearMin = _.min(data);
			return {year: k, data: data, mean: yearAve, max: yearMax, min: yearMin};
		});
  	this.data.meanTemp = {data: dataset.reverse() , extent: [0, 400]};


  	d3.select('body').style('height', this.bodyHeight + 'px');
  	this.windowHeight = $(window).height();
  	this.scrollScale = d3.scale.linear().domain([0, this.bodyHeight - this.windowHeight * 0.99]).range([0, 64]).clamp(true);
  ...
  updateInfo: function() {
    ...
    html += '<p>Maximum hour: ' + d.max + ' units</p>';
    html += '<p>Minimum hour: ' + d.min + ' units</p>';
    html += '<p>Day average: ' + d.mean.toFixed(1) + ' units</p>';
  ...
  initChart: function() {
    ...
    years
      .append('line')
      .classed('end', true)
      .attr('x1', this.xScale(6))
      .attr('y1', this.yScale(0))
      .attr('x2', this.xScale(6))
      .attr('y2', function(d) {return that.yScale(d.data[6]);});

    years
      .append('text')
      .classed('label', true)
      .attr('x', this.xScale(6) + 5)
...
d3.json('data/weekly_hoodies.json', function(temperatureData) {
