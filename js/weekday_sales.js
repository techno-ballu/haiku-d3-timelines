...
var chart = {
  ...
  init: function() {
  	...
  	this.data.meanTemp = {data: dataset.reverse() , extent: [-2, 40]};


  	d3.select('body').style('height', this.bodyHeight + 'px');
  	this.windowHeight = $(window).height();
  	this.scrollScale = d3.scale.linear().domain([0, this.bodyHeight - this.windowHeight * 0.99]).range([0, 97]).clamp(true);
  ...
  updateInfo: function() {
    ...
    html += '<p>Maximum hour: ' + (d.max*100).toFixed(1) + ' units</p>';
    html += '<p>Minimum hour: ' + (d.min*100).toFixed(1) + ' units</p>';
    html += '<p>Day average: ' + (d.mean*100).toFixed(1) + ' units</p>';
...
d3.json('data/daily_hoodies.json', function(temperatureData) {
